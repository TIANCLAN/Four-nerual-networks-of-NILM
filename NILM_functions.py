...
def load_data(model, appliance, dataset, width, strides, test_from=0, set_type="both"):
    ...
    def import_data(app_type, house):
        ...
        x = np.load("../../data/{}_main_house_{}.npy".format(app_type, house))
        y = np.load("../../data/{}_appliance_house_{}.npy".format(app_type, house))
...
def FEE_metric(y_total_pred, y_total_true):
    
    # print("-------------FEE-BEGIN---------------")
    pred_correct = 0
    # print("y_total_shape",y_total_pred.shape)
    FEE = np.zeros(y_total_pred.shape[1])
    # print("----test----")
    for num in range(y_total_pred.shape[1]):
        if y_total_pred[0][num] <= y_total_true[0][num]:
            pred_correct += y_total_pred[0][num]
        elif y_total_pred[0][num] > y_total_true[0][num]:
            pred_correct += y_total_true[0][num]
    # print("test---")
    # print(pred_correct)
    # print(y_total_true.sum())
    FEE = pred_correct/(y_total_true.sum())
    print(FEE)
    # print("y_total_true.sum()",y_total_true.sum())
    # print("pred_correct",pred_correct)
    # print("y_total_true",y_total_true)
    # print("pred_correct/(y_total_true.sum())",FEE)
    # print("-------------FEE-end---------------")
    return FEE
